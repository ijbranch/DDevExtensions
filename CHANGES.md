...
## 2026-05-12 - v3.16.5 - Delphi 13 Win64 Build and 64-bit IDE Install Support

**Problem:** Delphi 13 ships both a 32-bit IDE host (`Studio\37.0\bin\bds.exe`) and a 64-bit IDE host (`Studio\37.0\bin64\bds.exe`). The existing build only produced a 32-bit DLL (`DDevExtensionsD130.dll`), which the 64-bit IDE cannot load. The 32-bit IDE must keep working unchanged.

**Changes Made:**
1. `Code/DDevExtensions/D_D130/DDevExtensions.dproj`: Removed the unconditional `<DCC_DCCCompiler>DCC32</DCC_DCCCompiler>` that forced Win32 linking. Added `Cfg_1_Win64` / `Cfg_2_Win64` build-configuration groups and a `Base_Win64` group that points `Debugger_HostApplication` at `$(BDS)\Bin64\bds.exe`, clears the Win32 image base (`53900000` is a low x86 address that's pointless on x64), and overrides `DCC_DependencyCheckOutputName` to `..\bin\DDevExtensionsD130x64.dll`. Added Win64-specific `Cfg_*_Win64` `VerInfo_Keys` entries naming `DDevExtensionsD130x64.dll`, plus Release/Debug Win64 `PreBuildEvent` groups that copy `CompileInterceptorWx64.dll` into the output folder.
2. `Code/DDevExtensions/D_D130/DDevExtensions.dpr`: Made `$LIBSUFFIX` platform-aware — `'D130x64'` on Win64, `'D130'` on Win32 — so the two builds emit distinct DLL filenames and coexist in `bin\`.
3. `CompileInterceptor/Source/CompileInterceptorW.dproj`: Removed forced `DCC32`, added `Cfg_1_Win64` / `Cfg_2_Win64` groups, set the Win64 `DCC_DependencyCheckOutputName` to `..\bin\CompileInterceptorWx64.dll`, cleared the Win32 `DCC_ImageBase` for Win64, and added Win64 version-info keys with the new file name.
4. `CompileInterceptor/Source/CompileInterceptorW.dpr`: Added `{$IFDEF WIN64} {$LIBSUFFIX 'x64'} {$ENDIF}` so the Win64 helper builds as `CompileInterceptorWx64.dll`.
5. `Code/DDevExtensions/Source/CompileProgress/CompileProgress.pas`: The `HookedProjectGroupCompileActive` routine used inline x86 asm (`push eax`, `mov eax,[esp+8]` etc.) to compare the call's return address against `OrgAppBuilderCompile` and pick between two jump targets. That trick does not translate to Win64 calling conventions. Wrapped the asm body in `{$IFDEF CPUX86}` and provided a Pascal Win64 fallback that always tail-calls `CompileActiveProject` (the enhanced-dialog handler). Forward declaration is now `procedure` on x86 / `function(Instance; CompileMode; Wait): Boolean` on Win64 to match the new body.
6. `Code/DDevExtensions/Source/DSUFeatures/DisableAlphaSortClassCompletion.pas`: The whole unit relies on scanning the 32-bit IDE binary for byte signatures (`CompleteSetSortedBytes`, `CompleteMethodSymbolTableIteratorBytes`) and patching x86 calls. Wrapped the entire implementation in `{$IFDEF CPUX86}` and added a no-op stub `InstallDisableAlphaSortClassCompletion(Value)` for Win64. The feature is unavailable on the 64-bit IDE host; the rest of DDevExtensions still works.
7. `Code/DDevExtensions/Source/Editor/DocModuleHandler.pas`: Similar story — this unit dispatches through the IDE's `TDocModule` VMT using x86-asm trampolines (`Call`, `Call1`, `Call2` with `push ebx`, `xchg eax,edx`, …) and `external coreide_bpl name '@Docmodul@…'` declarations whose mangled C++ names target the 32-bit BPL. Wrapped the implementation in `{$IFDEF CPUX86}` and added Win64 stubs that satisfy every method body declared in the interface (returning `False`/`''`/`nil` defaults) plus an `InitDocModuleHandler` that returns `False`. The Reload-Files dialog still compiles and shows; its "reload via TDocModule internals" path is inactive on Win64.
8. `Code/DDevExtensions/Installer/Main.pas`: Extended `TEnvData` with `ExpertsSubKey` (`Experts` vs `Experts x64`), `HostExeRelPath` (`bin\bds.exe` vs `bin64\bds.exe`), and `CompInterceptorDll` so the installer can place the right helper DLL next to the plugin. Renamed the existing D130 entry to "RAD Studio 13.0 (32-bit IDE)" and added a new `ekDelphi130x64` entry "RAD Studio 13.0 (64-bit IDE)" using `Experts x64` (Embarcadero's convention — note the space, matching `Disabled Packages x64`, `Environment Variables x64`, and what GExperts/MMX/CnWizards/etc. use) under the same `Embarcadero\BDS\37.0` product. `FormCreate` now probes `RootDir\<HostExeRelPath>` instead of hard-coding `bin\bds.exe`, `RegisterExpert` / `UnregisterExpert` / `HasExpert` use `ExpertsSubKey`, and `DoInstall` / `DoUninstall` copy / remove `EnvData.CompInterceptorDll` instead of the hard-coded `CompileInterceptorW.dll`.
9. `Code/DDevExtensions/build.bat`: Split the "Delphi 13.0" section into Win32 and Win64 passes. The Win64 pass first rebuilds `CompileInterceptorW.dproj` as Win64 (so the Win64 PreBuildEvent can copy `CompileInterceptorWx64.dll`), then builds `DDevExtensions.dproj` with `/p:Platform=Win64`.
10. **`delayed` import directive added to all 60 static BPL external declarations** across 14 source files (`CompilerClearOtherStates.pas`, `IDEUtils.pas`, `FrmeOptionPageDSUFeatures.pas`, `NativeProgressForm.pas`, `ProjectResource.pas`, `ToolsAPIHelpers.pas`, `DocModuleHandler.pas`, `DisableAlphaSortClassCompletion.pas`, `FrmReloadFiles.pas`, `CodeInsightHandling.pas`, `StartParameterManagerReg.pas`, `OldPalette.pas`, `FrmeOptionPageKeybindings.pas`, `FrmeOptionPageUnitSelector.pas`). Without `delayed`, every `external coreide_bpl name '@Foo@Bar$qqrv'` (and equivalents against `delphicoreide_bpl`, `dbkdebugide_bpl`, etc.) is bound at DLL-load time via the import descriptor; if any one fails, the IDE shows "Entry Point Not Found" and refuses to load the wizard. The mangled C++ ABI symbol names are 32-bit-only, so every static import would have failed on the 64-bit IDE host. Marking them `delayed` shifts resolution to first call, lets the DLL load successfully on Win64, and pushes failures into the call sites of features we already no-op on Win64 (the `DocModuleHandler` and `DisableAlphaSortClassCompletion` stubs short-circuit before they reach any import).
11. `Code/DDevExtensions/Installer/Main.pas`: Corrected the 64-bit IDE's `ExpertsSubKey` from `Experts64` to `Experts x64` (with a space) — that is the registry subkey Embarcadero's 64-bit `bds.exe` host actually reads, matching the `Disabled Packages x64` / `Environment Variables x64` convention used by every other plugin (GExperts, MMX, CnWizards, JetPascal, etc.).
12. `Code/DDevExtensions/Source/CompileInterceptor/Source/InterceptLoader.pas`: Made the helper-DLL filename platform-aware. On Win64 the loader now looks for `CompileInterceptorWx64.dll`; on Win32 it still looks for `CompileInterceptorW.dll`. Previously the 64-bit DLL would raise "Cannot find CompileInterceptorW.dll" at startup.
13. `Code/DDevExtensions/Source/VirtTreeHandler.pas`: `TIDEVirtualTreeHandler.InitMethods` no longer raises during initialisation when a `TreeImport` symbol lookup against `vclide370.bpl` returns nil (the 64-bit BPL uses different C++ name mangling). On Win64 the field is silently left pointing at `@NotSupported` — only an actual call to the missing accessor will raise, not the constructor itself.
14. `Shared/Hooking.pas`: `RedirectOrgCall` on Win64 changed from `raise Exception.Create('RedirectOrgCall is not supported in x64 mode, yet')` to a silent stub returning `nil`. The original raise blocked plugin startup the moment any feature tried to install a hook; returning nil lets the surrounding `@OrgCallFoo := RedirectOrgCall(...)` pattern complete and the feature degrade silently.
15. `Code/DDevExtensions/Source/DSUFeatures/FrmeOptionPageDSUFeatures.pas` (`SetShowFileProjectInPrjMgr`) and `Code/DDevExtensions/Source/DSUFeatures/StrucViewSearch.pas` constructor: Gated both `TIDEVirtualTreeHandler.Create` injection sites on `{$IFNDEF CPUX64}`. The "Show file project in Project Manager" column and the Structure-View search combobox both attach Pascal callbacks to the IDE's virtual trees and route every paint cycle through the tree handler; on Win64 the handler's accessors are `@NotSupported` stubs, so every IDE paint raised "Not Supported". Both features are silently inactive on Win64 — the shared XML config keeps the user's setting for the 32-bit IDE.

**Scope:** Win64 support is **built and tested on Delphi 13.1 Florence only**. Older `D_Dxxx` projects (`D_D102` … `D_D120`) remain Win32-only by design — their `.dproj` files still force `DCC32` and no Win64 PreBuildEvent is wired up there. The 64-bit IDE has a separate entry in the installer and writes under `Experts x64`, so the 32-bit IDE install is untouched and both bitnesses can be installed side-by-side.

**Files Modified:** `Code/DDevExtensions/D_D130/DDevExtensions.dproj`, `Code/DDevExtensions/D_D130/DDevExtensions.dpr`, `CompileInterceptor/Source/CompileInterceptorW.dproj`, `CompileInterceptor/Source/CompileInterceptorW.dpr`, `Code/DDevExtensions/Source/CompileProgress/CompileProgress.pas`, `Code/DDevExtensions/Source/DSUFeatures/DisableAlphaSortClassCompletion.pas`, `Code/DDevExtensions/Source/Editor/DocModuleHandler.pas`, `Code/DDevExtensions/Installer/Main.pas`, `Code/DDevExtensions/build.bat`
